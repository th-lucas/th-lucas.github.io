...
function updatePatterns4(bestSongs) {
	var p = container4
		.select('.bestSongsDefs')
		.selectAll('pattern')
		.data(bestSongs);  
	...
	p.enter()
		...
		.attr('id', function(d) { 
			...
			return camelize(d['Artist']) + '-best-song-img';
		})
...
function updateBestSongs(bestSongs) {
	...
	u.enter()
		...
		.style('fill', function(d, i) {
			return 'url(#' + camelize(d['Artist']) + '-best-song-img)';	
		});
...
function decadeButtonsShowHideCircles(){
	...
	container4.select('.averageLines')
...
function bestSongsButtonFunction(){
...
decadeButtons.on('click', function(){ 
	bestSongsButtonFunction();
	allButton.classed('active', false);
	allButton.classed('inactive', false);
	allButton.classed('active_bis', false);
	container4.select('.averageLines')
...
d3.selectAll('.band-img').on('click', function(){
	...
	if(d3.select(this).classed('active-artist')){
		...
		var tracksToDisplay = container4.select('.circles')
								.selectAll('circle')
								.filter(function() { 
		...
		if(!d3.select('.btn-all').classed('active')){
			d3.select('#decadeSelector').selectAll('label:not(.active_bis)').each(function(){
				val = d3.select(this).select('input').node().value;
				container4.select('.circles').selectAll('circle.' + val).style('opacity', 0);
			});
		}
		bestSongsButton.attr('disabled', true);
	}
